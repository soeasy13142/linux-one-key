...
MSG_GUARD_SYMLINK="拒绝写入 %s：目标文件是符号链接（可能被劫持）"
MSG_GUARD_NOT_REGULAR="拒绝写入 %s：目标不是常规文件"
MSG_GUARD_SIZE="拒绝写入 %s：大小 %s 字节超过上限 %s 字节"
MSG_GUARD_LINES="拒绝写入 %s：行数 %s 超过上限 %s"
...
MSG_VERSION_LOG_1="v1.9.0 (2026-08-18) 科技lion 借鉴批次：--version / 写入护栏 / 状态菜单（827 Bats）"
MSG_VERSION_LOG_2="v1.8.0 (2026-08-14) Lite 运行痕迹清理"
MSG_VERSION_LOG_3="v1.7.0 (2026-08-14) sudo+日志加固 + check.sh CIS/STIG 扫描器"
MSG_VERSION_LOG_4="v1.6.0 (2026-08-01) 备份/回滚中心 + 安全仪表盘 + 更换软件源"
